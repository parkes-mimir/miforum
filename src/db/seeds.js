...
function initDefaultData(db) {
  ...
  if (!officialChannel) {
    ...
    `
      ...
      .run();
    officialChannelId = result.lastInsertRowid;
    ...
    officialChannelId = officialChannel.id;
  }
  ...
  if (catCount === 0) {
    const insert = db.prepare(`
      INSERT INTO categories (name, label, description, color, icon, section_type, channel_id, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const cat of DEFAULT_CATEGORIES) {
      insert.run(
        ...
        cat.color,
        cat.icon || '',
        cat.section_type || 'normal',
        officialChannelId,
        cat.sort_order
      );
    }
  } else {
    // 迁移：更新已有分类的 section_type 和 channel_id
    const notice = db.prepare("SELECT id FROM categories WHERE name = 'notice'").get();
    if (notice) {
      db.prepare(
        "UPDATE categories SET section_type = 'announcement', icon = '📢', description = '官方公告和通知' WHERE name = 'notice' AND (section_type IS NULL OR section_type = 'normal')"
      ).run();
    }
    const hot = db.prepare("SELECT id FROM categories WHERE name = 'hot'").get();
    if (!hot) {
      db.prepare(
        'INSERT INTO categories (name, label, description, color, icon, section_type, channel_id, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
      ).run('hot', '热门', '热门帖子自动聚合', 'bg-red-100 text-red-700', '🔥', 'hot', officialChannelId, 2);
    }
    // 将未关联频道的板块归入官方频道
    db.prepare('UPDATE categories SET channel_id = ? WHERE channel_id IS NULL').run(officialChannelId);
  }

  // 所有用户自动成为官方频道成员（如果还不是）
  const allUsers = db.prepare('SELECT id FROM profiles').all();
  const insertMember = db.prepare('INSERT OR IGNORE INTO channel_members (channel_id, user_id, role) VALUES (?, ?, ?)');
  for (const user of allUsers) {
    insertMember.run(officialChannelId, user.id, 'member');
  }
}
