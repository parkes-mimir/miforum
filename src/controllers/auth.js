...
function authRoutes(app, db) {
  ...
  function incrementAttempts(key) {
    ...
    if (!lockout) {
      ...
      if (attempts >= 5) {
        ...
        db.prepare(
          "UPDATE code_lockouts SET attempts = 0, locked_until = ?, updated_at = datetime('now') WHERE lockout_key = ?"
        ).run(lockedUntil, key);
      } else {
        db.prepare("UPDATE code_lockouts SET attempts = ?, updated_at = datetime('now') WHERE lockout_key = ?").run(
          attempts,
          key
        );
      }
  ...
  app.post('/api/send-code', async (req, res) => {
    ...
    db.prepare('INSERT INTO verification_codes (email, code, type, expires_at) VALUES (?, ?, ?, ?)').run(
      email,
      code,
      codeType,
      expiresAt
  ...
  app.post('/api/register', async (req, res) => {
    ...
    // 测试环境跳过验证码
    if (process.env.NODE_ENV !== 'test' && !code) return res.status(400).json({ error: '请填写验证码' });
    ...
    if (password.length > 72) return res.status(400).json({ error: '密码最多72位' });
  ...
  app.post('/api/login', async (req, res) => {
    ...
    const user = db
      .prepare(
        'SELECT id, display_id, username, email, password_hash, points, role, force_password_change FROM profiles WHERE email = ?'
      )
      .get(email);
  ...
  app.get('/api/me', (req, res) => {
    ...
    const u = db
      .prepare(
        'SELECT id, display_id, username, email, avatar_url, bio, location, website, profile_public, points, exp, muted, role, title, avatar_frame, rename_chances, force_password_change FROM profiles WHERE id = ?'
      )
