...
- **图片排序和自定义位置**：
  - 上传图片后每张显示序号，悬停显示"插入正文"和"删除"按钮
  - 点击"插入正文"在光标位置插入 `[img:N]` 占位符
  - 支持文字-图片-文字-图片交错排版
  - 图片支持拖拽排序
  - 帖子渲染自动识别 `[img:N]` 占位符，内联显示图片
  - 无占位符的旧帖子兼容（图片显示在内容上方）
  - 帖子列表预览自动过滤图片占位符
- **编辑模式图片管理**：新增图片也有插入按钮

### 修复

- 图片文件名冲突导致覆盖（加随机后缀 `crypto.randomBytes`）
- 图片插入按钮改为始终可见（之前 hover 才显示）

### 涉及文件

| 文件 | 改动 |
|------|------|
| src/utils/upload.js | 文件名加随机后缀防冲突 |
| public/forum.html | 图片序号+插入按钮、拖拽排序、[img:N] 渲染 |
| public/post.html | 编辑模式图片插入、[img:N] 渲染 |
